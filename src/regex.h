...
struct regex_flags_t {
    ...
    regex_flags_t(const regex_flags_t &other)
        : raw_data(other.raw_data)
    {}

    regex_flags_t &operator=(const regex_flags_t &other) {
        raw_data = other.raw_data;
        return *this;
    }
    ...
    /** Returns pointer to flags bits.
     */
    const bits_t *operator->() const {return &bits;}

    /** Returns pointer to flags bits.
     */
    bits_t *operator->() {return &bits;}

    /** Comparison operator.
     */
    friend bool operator==(const regex_flags_t &lhs, const regex_flags_t &rhs) {
        return lhs.raw_data == rhs.raw_data;
    }

protected:
    union {uint8_t raw_data; bits_t bits;}; //!< flags storage
    static_assert(sizeof(raw_data) >= sizeof(bits), "fix sizes");
};

/** Converts set of bool to pcrepp flags.
 */
inline uint32_t to_pcre_options(regex_flags_t flags) {
    uint32_t result = 0;
...
public:
    ...
    /** Returns captured characters as a string view.
     */
    string_view_t view() const {return {begin(), end()};}
...
public:
    ...
    RegexCapture_t operator[](size_type i) const {
        return {
            captures[2 * i],
            subject.data() + captures[2 * i],
            subject.data() + captures[2 * i + 1]
        };
    }
...
protected:
    ...
    int do_match(code_pointer code, options_type options, size_type i) {
        ...
        return pcre2_match(
            code,
            reinterpret_cast<string_type>(subject.data()),
            subject.size(),
            i,
            options,
            match_data.get(),
    ...
    string_view_t subject;   //!< the subject of matching
...
public:
    ...
    /** C'tor.
     */
    Regex_t(string_view_t re, regex_flags_t flags_value = {})
        : pattern_string(re.begin(), re.end()),
          options(to_pcre_options(flags_value)), flags_value(flags_value),
          code(compile(re, options)), endl_value(detect_endl(code))
    {}
    ...
    /** Returns regular expression expression as a string.
     */
    const std::string &pattern() const {return pattern_string;}

    /** Returns flags used to compile/match the regular expression.
     */
    const regex_flags_t flags() const {return flags_value;}

    /** Returns first match.
     */
    RegexMatch_t match(string_view_t subject) const {
     ...
     */
    std::string replace(string_view_t subject, string_view_t with) const {
        std::string result;
        ...
        if (auto match_value = match(subject)) do {
            ...
            if (prefix_size > 0)
                result.append(subject.data(), subject_offset, prefix_size);
        ...
        if (suffix_size > 0)
            result.append(subject.data(), subject_offset, suffix_size);
...
protected:
    ...
    void expand_referencies(
        ...
        for (auto &ref: references) {
            ...
            if (prefix_size > 0)
                result.append(with.data(), with_offset, prefix_size);
        ...
        if (suffix_size > 0)
            result.append(with.data(), with_offset, suffix_size);
    }
    ...
    compile(
        ...
        return {
            pcre2_compile(
                reinterpret_cast<string_type>(pattern.data()),
                pattern.size(),
                options,
                &error.code,
                &error.offset,
     ...
     */
    static code_type compile(string_view_t pattern, options_type options) {
        RegexError_t error = {pattern.data(), 0, 0};
    ...
    options_type options;       //!< the pcre2 options
    regex_flags_t flags_value;  //!< the regular expression flags
    code_type code;             //!< the pcre2 native code of the regular expr
    endl endl_value;            //!< what kind of endline is used in pattern
};

inline RegexMatch_t::RegexMatch_t(
    const Regex_t *re,
    string_view_t subject,
    size_type i,
    options_type options
): re(re), subject(subject), captures(nullptr),
   captures_size(0), match_data(make_match(re))
{do_match(re->options | options, i);}

inline RegexMatch_t::match_type RegexMatch_t::make_match(const Regex_t *re) {
    if (auto match_data = make_match(re->code.get()))
        return match_data;
    throw std::bad_alloc();
}

inline void RegexMatch_t::do_match(options_type options, size_type i) {
...
inline RegexMatch_t RegexMatch_t::next() const {
    // there is no 'next' matches if global is turned off
    if (!re->flags()->global)
        return {re};
...
RegexMatch_t::next_after_empty(size_type next_start_i) const {
    ...
    if (auto match = RegexMatch_t(re, subject, next_start_i, options))
        return match;
