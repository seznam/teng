 ...
 */
void open_frag(Context_t *ctx, const Pos_t &pos, IVariable_t sym);
...
/** Discards all instructions generated by invalid expression from given
 * address untill end of program.
 */
 ...
 * 5. Finalizing
 *
 * You could noticed that jump instruction has set invalid offsets. The can't
 ...
 */
void open_format(Context_t *ctx, const Pos_t &pos, const FormatOptions_t &opts);
 ...
 */
void set_var(Context_t *ctx, IVariable_t var_name);
...
/** Generates lookup to dictionary instruction.
 */
inline void generate_print_dict_lookup(Context_t *ctx, const Token_t &token) {
 ...
 */
inline void generate_print_dict_undef(Context_t *ctx, const Token_t &token) {
