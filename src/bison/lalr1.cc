...
b4_location_define])])[
  ...
  private:
    ...
    virtual void yystack_print_ (const char *);
...
[
  ...
  yyerrlab1:
    yyerrstatus_ = 3;   // Each real token shifted decrements this.
    {
      ...
      if (yydebug_)
      yystack_print_("STACK shift error");
    }
    goto yynewstate;
...
#undef YYCASE_
  ...
  // Print the state stack on the debug stream.
  void
  ]b4_parser_class_name[::yystack_print_ (const char *prefix)
  {
    *yycdebug_ << prefix;
